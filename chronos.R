...
## Mods by SDWF
## August 10, 2015: add in fitting of multiple models and saving of the model with lowest PHIIC
...
tree.file <- "LASV_rtt.nwk"#args[1]
out.file <- "LASV_chronos"#args[2]
ncatmax <- 10#as.integer(args[3])
...
while(ncat<=ncatmax){
  ...
  newPHIIC <- attr(dated.tree,"PHIIC")$PHIIC
  results[[paste("D",ncat,sep="")]] <- dated.tree
  if(newPHIIC<PHIIC){
    i <- i+1
    ncat <- ncat+1
    PHIIC <- newPHIIC
...
results.tbl <- data.frame(matrix(unlist(lapply(results,attr,"PHIIC")),ncol=3,byrow=T))
results.tbl <- cbind(paste("D",seq(1,ncat),sep=""),results.tbl)
names(results.tbl) <- c("Model","logLik","k","PHIIC")
results.tbl$Model <- as.character(results.tbl$Model)

bestmodel <- results.tbl$Model[results.tbl$PHIIC==min(results.tbl$PHIIC)]

pander(results.tbl)

write.table(results.tbl,file=paste(out.file,".txt",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
write.tree(results[[bestmodel]],file=paste(out.file,".nwk",sep=""))